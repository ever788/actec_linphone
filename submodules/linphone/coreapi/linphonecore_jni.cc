...
extern "C" void Java_org_linphone_core_LinphoneCoreImpl_sendoptions(JNIEnv *env,jobject thiz,jlong ptr,jlong addr){
	LinphoneCore *lc = (LinphoneCore *)ptr;
	linphone_core_ping_options(lc,(const LinphoneAddress *)addr);
}

extern "C" void Java_org_linphone_core_LinphoneCoreImpl_addheard_sendoptions(JNIEnv *env,jobject thiz,jlong ptr,jlong addr,jstring jhear,jstring jreal){
	LinphoneCore *lc = (LinphoneCore *)ptr;
	const char* chear = GetStringUTFChars(env, jhear);
	const char* creal = GetStringUTFChars(env, jreal);
	linphone_core_ping_add_heard_options(lc,(const LinphoneAddress *)addr,chear,creal);
	ReleaseStringUTFChars(env, jhear,chear);
    ReleaseStringUTFChars(env, jreal,creal);
}
