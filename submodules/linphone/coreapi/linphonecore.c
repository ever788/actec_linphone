...
void linphone_core_ping_options(LinphoneCore *lc,const LinphoneAddress *addr){
	SalOp *t=sal_op_new(lc->sal);
	const char *from=NULL;
	LinphoneProxyConfig *proxy=NULL;
	proxy=linphone_core_lookup_known_proxy(lc,addr);
	if (proxy!=NULL) {
	...
	char * real_url=linphone_address_as_string(addr);
	sal_ping(t,from,real_url);
	if (real_url!=NULL) ms_free(real_url);
	sal_op_release(t);
}

void linphone_core_ping_add_heard_options(LinphoneCore *lc,const LinphoneAddress *addr,const char*hear,const char*real){
	SalOp *t=sal_op_new(lc->sal);
	const char *from=NULL;
	LinphoneProxyConfig *proxy=NULL;
	proxy=linphone_core_lookup_known_proxy(lc,addr);
	...
	from=linphone_core_get_primary_contact(lc);
	char * real_url=linphone_address_as_string(addr);
	sal_options_add_headerping(t,from,real_url,hear,real);
	if (real_url!=NULL) ms_free(real_url);
	sal_op_release(t);
}
