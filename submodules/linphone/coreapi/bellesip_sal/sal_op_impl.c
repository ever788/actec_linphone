...
belle_sip_request_t* sal_op_build_request(SalOp *op,const char* method) {
	...
	if (sal_op_get_call_id(op)) {
		...
		req=belle_sip_request_create(
							...
							method,
							call_id_header,
							belle_sip_header_cseq_create(m_sequence++,method),
							from_header,
							to_header,
...
belle_sip_request_t* sal_op_build_options_request(SalOp *op,const char* method,char* hear,char *value) {
	belle_sip_header_from_t* from_header;
	belle_sip_header_to_t* to_header;
	belle_sip_provider_t* prov=op->base.root->prov;
	belle_sip_request_t *req;
...
int sal_options_add_headerping(SalOp *op, const char *from, const char *to,const char *heard,const char *value){
	sal_op_set_from(op,from);
	sal_op_set_to(op,to);
	belle_sip_request_t* trequest=sal_op_build_request(op,"OPTIONS");
	belle_sip_header_t*	 theader=belle_sip_header_create(heard, value);
	belle_sip_message_add_header(BELLE_SIP_MESSAGE(trequest),theader);
	int resulet=sal_op_send_request(op,trequest);
	return resulet ;
}
